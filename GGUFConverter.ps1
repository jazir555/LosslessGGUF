...
if ($IsWindows) { $cmakeArgs += "-D", "CMAKE_INSTALL_PREFIX=""$binDir""" }
else { $cmakeArgs += "-D", "CMAKE_INSTALL_PREFIX=$binDir" }
if (-not $cudaPath) {
...
Log "Building lz5_v2.3_converter.exe with CMake..."
...
$btnConvert.Dock = 'Top'; $btnConvert.Text = 'Convert → "LZ5 v2.3" GGUF'; $btnConvert.Height = 40
...
$btnConvert.Add_Click({
    ...
    $outName = [IO.Path]::GetFileNameWithoutExtension($model) + '_LZ5v2.3.gguf'
    $outFile = Join-Path $outDir $outName
    $prg.Value = 0; $btnConvert.Enabled = $false; $btnCancel.Enabled = $true
    Log "Starting LZ5 v2.3 conversion → $outFile"
...
$timer.Add_Tick({
    ...
    if ($global:job.State -eq 'Running') {
        ...
        if ($log) {
            foreach ($l in $log) {
                if ($l -like "LZ5_PROGRESS::*") {
                    ...
                    $progressVal = [int]$progressStr
                    if ($progressVal -ge 0 -and $progressVal -le 100) {
                        $prg.Value = $progressVal
                    }
                } else {
                    Log $l
                }
        ...
        if ($global:job.State -eq 'Failed') { Log 'Conversion failed – see log above' } else {
            $outFile = Join-Path $outDir ([IO.Path]::GetFileNameWithoutExtension($tbModel.Text) + '_LZ5v2.3.gguf')
...
Log 'LZ5 v2.3 Converter ready. Select a .safetensors model and click Convert.'
